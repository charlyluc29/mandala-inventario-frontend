...
function InventarioGeneral() {
  ...
  // =========================
  // ELIMINAR PRODUCTO (SIN HISTORIAL)
  // =========================
  const eliminarProducto = async item => {

    if (
      !confirm(
        `¿Eliminar "${item.producto?.nombre}" de ${item.sucursal?.nombre}?`
      )
    ) return

    try {

      // DELETE directo (no crea movimiento)
